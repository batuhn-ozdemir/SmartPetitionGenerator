...
public class ApiRequestSignatureFilter extends OncePerRequestFilter {
    ...
    // Shared secret used to verify Android request signatures.
    @Value("${app.security.signing-secret:}")
    private String signingSecret;

    // Maximum allowed time difference between client and server timestamps.
    @Value("${app.security.max-skew-seconds:300}")
    private long maxSkewSeconds;

    private final ObjectMapper objectMapper = new ObjectMapper();

    // Only protect petition-related API endpoints.
    @Override
    ...
    protected void doFilterInternal(
        ...
        // Reject all protected requests if the backend secret is not configured.
        ...
        try {
            ...
            reject(
                    response,
                    HttpServletResponse.SC_UNAUTHORIZED,
                    "X-Timestamp geçerli bir epoch saniyesi olmalıdır."
            );
            return;
        }

        // Reject old or future timestamps to reduce replay attacks.
        ...
        // Rebuild the exact payload that Android signed.
        String payload =
                request.getMethod() + "\n" +
        ...
        // Signature is valid, continue to the controller.
    ...
    // Constant-time comparison to avoid timing-based signature guessing.
    ...
    // Calculates HMAC-SHA256 signature in hexadecimal format.
